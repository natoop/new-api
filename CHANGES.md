...
2. 协议文案与版本号已预置（默认协议见附录 A，版本号默认 v1.0），仅需在法务设置开启「控制台强制签署」即可上线签约；文案可在面板自定义覆盖（option 表的值优先于代码默认值）
3. 虎皮椒：支付设置中配置对应通道 appid/appsecret；微信/支付宝按配置自动出现在购买面板
4. 促销码次数为下单时预占：挂起未付的 pending 订单会占用一次名额直到过期自动回补（属预期行为）
5. Stripe 促销折扣经确定性 Coupon（`promo-bps-<bps>`，按折扣比例复用）实现，仅首期生效；同一折扣比例只会在 Stripe 后台产生一个 Coupon 对象

---

## 附录 A：默认《用户服务协议》文案

> 本文案已作为 `legal.user_agreement` 的**代码默认值**预置（`setting/system_setting/legal.go`），版本号默认 v1.0。管理员只需在「系统设置 → 法务」开启控制台强制签署即可上线；如需自定义，在面板修改后 option 表的值优先于此默认文案。建议正式启用前由法律顾问复核。
