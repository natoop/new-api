...
func TestLogFixedMeteredTaskConsumptionWritesTerminalStandardLog(t *testing.T) {
	...
	usedQuota, requestCount := getUserUsageAccounting(t, userID)
	assert.Equal(t, 4000, usedQuota)
	assert.Equal(t, 1, requestCount)
	assert.Equal(t, int64(4000), getChannelUsedQuota(t, channelID))
...
func TestRefundTaskQuota_FixedMeteredDoesNotReverseUncommittedUsage(t *testing.T) {
	truncate(t)
	ctx := context.Background()

	const userID, tokenID, channelID = 91, 91, 91
	const reservedQuota = 2000
	seedUser(t, userID, 6000)
	seedToken(t, tokenID, userID, "sk-fixed-metered-refund", 4000)
	seedChannel(t, channelID)
	require.NoError(t, model.DB.Model(&model.Token{}).Where("id = ?", tokenID).
...
func TestSettle_FixedMeteredSkipsRecalculation(t *testing.T) {
	truncate(t)
	ctx := context.Background()

	const userID, tokenID, channelID = 92, 92, 92
	const reservedQuota = 5000
	seedUser(t, userID, 10000)
	seedToken(t, tokenID, userID, "sk-fixed-metered-settle", 8000)
	seedChannel(t, channelID)
	...
	settleTaskBillingOnComplete(ctx, &mockAdaptor{adjustReturn: 3000}, task, &relaycommon.TaskInfo{TotalTokens: 9999})

	assert.Equal(t, reservedQuota, task.Quota)
