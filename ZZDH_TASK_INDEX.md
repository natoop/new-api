...
The dated files are evidence. Runtime configuration remains in new-api model pricing and the future data-driven ZZDH profile registry; never treat a catalogue snapshot as live configuration.
...
| Model admission | `relay/channel/task/zzdh/profile.go` | 47 confirmed video profiles: 16 V1 Seedance and 31 V8 Kling/Happyhorse/Wan/Minimax H3 |
| User-confirmed target scope | `ZZDH_VIDEO_CHANNEL_TASK.md` | 47 video-endpoint names plus 4 image-output names; five supplied names explicitly removed; endpoint contract and explicit H3 detail override determine the adapter track |
| Task lifecycle | Existing task table, polling, terminal CAS, failure refund | Reused; no new route or schema |
| Task pricing | Existing task pricing lifecycle | Video profiles return bounded `seconds` OtherRatios; model price remains in new-api configuration and existing pre-consume/settle/refund/audit logic is reused |
| Initial settlement policy | Implemented compatibility policy | Calculate from configured task terms and validated request/media metrics at submit; settle the accepted amount immediately; refund the persisted task quota on failure |
| Switcher frontend (only if later required) | `D:\code\goswtich\switcher\frontend` | Configuration/display only; never a second billing ledger |
...
| Where is the configured base price read? | `relay/helper/price.go` and `setting/ratio_setting/model_ratio.go` |
| How will non-token task pricing work? | `ZZDH_VIDEO_CHANNEL_TASK.md` independent-calculator contract; do not add model-specific arithmetic to the adaptor |
...
4. Configure the model's base price in new-api. Prices are configuration, not Go constants. Bind the model to a billing rule/profile; do not use a model price alone as evidence of protocol capability.
...
| 2026-08-06 | Phase 1 submit-time billing policy accepted | Reuse the original task-compatible shape: calculate configured task terms before submission, settle the accepted amount after submission, persist the task quota, and refund it on terminal failure; retain a later terminal-settlement migration point | User-confirmed requirement alignment; runtime not changed |
...
| Pending | Provider-neutral independent task calculator | Required only when a future model introduces a new metric or terminal-settlement rule; same-family additions continue through profile data plus pricing configuration | Not implemented; current `seconds` path reuses existing task billing |
